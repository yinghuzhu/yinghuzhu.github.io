...
任何外部 SaaS 服务都存在不可用的可能性。企业级身份提供商历史上发生过多次全球性服务中断：

- **2023 年 7 月**：Microsoft Azure AD 全球性中断，影响 Microsoft 365、Teams、Power Platform 等全部依赖其认证的服务
- **2024 年 1 月**：Microsoft 365 因网络路由变更导致全球性认证中断
...
![故障传播链路](./images/why-local-auth/01-risk-chain.png)
...
![IdP 迁移过渡期](./images/why-local-auth/03-idp-migration.png)
...
这个推导的前提假设是本地密码登录与联合登录面向相同的用户规模。在实际架构中，联合登录将全员密码安全的责任转移给上游 IdP，本地系统只需对个位数管理员负责。两个认证路径面向的用户群体存在显著差异：
...
联合登录将全员密码安全的责任转移给上游 IdP，本地系统只需对个位数管理员负责。
...
![认证路径收敛](./images/why-local-auth/02-auth-path-convergence.png)
...
将主力认证交给上游 IdP，同时保留极小范围的本地认证能力——这不是重复建设，而是认证架构的韧性取舍。
